...
{
    // internal static bool isLover(this PlayerControl player) => !(player == null) && (player == Lovers.lover1 || player == Lovers.lover2);

    [HarmonyPatch(typeof(ChatController), nameof(ChatController.SendChat))]
    private static class SendChatPatch
    {
        internal static bool Prefix(ChatController __instance)
        {
            ...
            if (AmongUsClient.Instance.NetworkMode == NetworkModes.FreePlay)
            {
                ...
                {
                    handled = true;
                    if (!int.TryParse(text.AsSpan(7), out int col))
                    {
                        __instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModResources.CommandErrorColor);
                    }
                    col = Math.Clamp(col, 0, Palette.PlayerColors.Length - 1);
                    CachedPlayer.LocalPlayer.PlayerControl.SetColor(col);
                    __instance.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModResources.CommandColorSucceed); ;
                }
            }
    ...
    internal static class EnableChat
    {
    ...
    internal static class SetBubbleName
    {
        internal static void Postfix(ChatBubble __instance, [HarmonyArgument(0)] string playerName)
        {
            ...
            if (CachedPlayer.LocalPlayer != null &&
                ...
                /*
                (Spy.spy != null && sourcePlayer.PlayerId == Spy.spy.PlayerId || Sidekick.sidekick != null && Sidekick.WasTeamRed && sourcePlayer.PlayerId == Sidekick.sidekick.PlayerId || Jackal.jackal != null && Jackal.wasTeamRed && sourcePlayer.PlayerId == Jackal.jackal.PlayerId)
                */
                __instance != null)
    ...
    internal static class AddChat
    {
        internal static bool Prefix(ChatController __instance, [HarmonyArgument(0)] PlayerControl sourcePlayer)
        {
            ...
            return localPlayer == null || MeetingHud.Instance != null || LobbyBehaviour.Instance != null || localPlayer.Data.IsDead /*|| localPlayer.isLover() && Lovers.enableChat*/ || sourcePlayer.PlayerId == CachedPlayer.LocalPlayer.PlayerId;
        }
    }
}